...
function initLobby(){
  ...
  // Live wins do Mines via BroadcastChannel + fallback localStorage events
  ...
  const appendWin = (payload) => {
    ...
    li.innerHTML = `<span class="badge">${when}</span> ${payload.user || 'Jogador'} ganhou <strong>${toBRL(payload.amount)}</strong> no Mines (x${payload.mult.toFixed(2)})`;
    winsList?.prepend(li);
    while(winsList && winsList.children.length>8){ winsList.removeChild(winsList.lastChild); }
  };

  try{
    const bc = new BroadcastChannel('mines-wins');
    bc.onmessage = (ev)=> { if(ev?.data?.type==='win') appendWin(ev.data); };
  }catch(e){ /* ignore in browsers sem suporte */ }

  window.addEventListener('storage', (e)=>{
    if(e.key==='mines:lastWin'){
      try{ const data = JSON.parse(e.newValue); if(data?.type==='win') appendWin(data); }catch(_){}
    }
  });
}
...
// Utilitário exportado global para páginas isoladas poderem usar
window.toBRL = window.toBRL || toBRL;
